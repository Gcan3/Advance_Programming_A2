...
def analyze():
    ...
    if entered_month < 1 or entered_month > 12 or entered_day < 1 or entered_day > 31 or format_year < 1980 or format_year > 2023:
    ...
    for games in range(gameLength):
        ...
        if format_team == source.json()['events'] [games] ['strHomeTeam']:
            # print("True")
            trueGameHome.append(games)
        elif format_team == source.json()['events'] [games] ['strAwayTeam']:
            # print("True")
            trueGameVisit.append(games)
        # else:
        #     print("false")

    #Collect the data of that specific event and store them in the respective variables
    if len(trueGameHome) > 0:
        home_team = source.json() ['events'] [trueGameHome[0]] ['strHomeTeam']
        visit_team = source.json() ['events'] [trueGameHome[0]] ['strAwayTeam']
        homeScore = int(source.json() ['events'] [trueGameHome[0]] ['intHomeScore'])
        visitScore = int(source.json() ['events'] [trueGameHome[0]] ['intAwayScore'])
    elif len(trueGameVisit) > 0:
        home_team = source.json() ['events'] [trueGameVisit[0]] ['strHomeTeam']
        visit_team = source.json() ['events'] [trueGameVisit[0]] ['strAwayTeam']
        homeScore = int(source.json() ['events'] [trueGameVisit[0]] ['intHomeScore'])
        visitScore = int(source.json() ['events'] [trueGameVisit[0]] ['intAwayScore'])
...
winner_label.place(x=470, y=420)
who_wins.place(x=590, y=421)
date_happen.place(x=650, y=470)
